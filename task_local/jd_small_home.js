...
!(async () => {
  ...
  $.inviteCodes = await getAuthorShareCode('https://ghproxy.com/https://raw.githubusercontent.com/ThorJsbox/QuanX/master/task_local/updateTeam/jd_updateSmallHomeInviteCode.json')
  if (!$.inviteCodes) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/ThorJsbox/QuanX@master/task_local/updateTeam/jd_updateSmallHomeInviteCode.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.inviteCodes = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/ThorJsbox/QuanX@master/task_local/updateTeam/jd_updateSmallHomeInviteCode.json')
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
